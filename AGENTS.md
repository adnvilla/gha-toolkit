...
.github/workflows/
  ...
  release.yml            # semantic-release runner using tools/release's locked npm toolchain
...
tests/                   # Shell-logic tests: extract a workflow step's `run:` body and run it stubbed
  ...
  release-toolchain.sh        # locked, script-free semantic-release bootstrap guard
.github/dependabot.yml    # grouped weekly updates for Action pins and tools/release npm dependencies
.github/actionlint.yaml   # actionlint config — ignore rules (see the job.workflow_* trap in section 11)
tools/release/            # versioned package.json/package-lock.json for release.yml's runtime dependencies
...
- `node`, `npm` and `npx` (to install and dry-run the locked semantic-release toolchain)
...
npx --prefix tools/release --no-install semantic-release --dry-run
```
...
5. Pin third-party actions to their full 40-character commit SHA, followed by the version tag in a
   comment (for example `actions/checkout@<sha> # v7`); Dependabot maintains those pins. Keep runtime
   npm dependencies in a committed lockfile and install them with `npm ci --ignore-scripts` (see
   `tools/release/` for `release.yml`).
...
- **Don't raise `conventional-changelog-conventionalcommits` past major 9** in `tools/release/` without
